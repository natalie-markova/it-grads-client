...
const FilterFunnel = ({ isActive }: FilterFunnelProps) => {
  ...
  useEffect(() => {
    ...
    // Создание случайных целей
    for (let i = 0; i < maxTargets; i++) {
      targets.push({
        x: Math.random() * 0.8 + 0.1, // 10-90% ширины
        y: Math.random() * 0.8 + 0.1, // 10-90% высоты
        locked: false,
        lockProgress: 0,
        distance: 0,
      })
    ...
    // Позиция целеуказателя (движется к ближайшей цели)
    let crosshairX = 0.5
    let crosshairY = 0.5
    let targetIndex = 0
    ...
    const draw = () => {
      ...
      // Движение целеуказателя к следующей цели
      ...
      targets.forEach((target, index) => {
        ...
        if (target.locked) {
          ...
          // Текущая цель в процессе захвата
          const color = target.lockProgress > 0 ? 'rgba(251, 191, 36, 0.8)' : 'rgba(59, 130, 246, 0.6)'

          // Круг захвата (заполняется по мере lockProgress)
